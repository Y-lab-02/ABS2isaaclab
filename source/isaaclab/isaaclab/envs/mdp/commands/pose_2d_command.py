...
class UniformPose2dCommand(CommandTerm):
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.object_avoid == True:
            tbd_envs = torch.tensor(env_ids, dtype=torch.long, device=self.device).clone()
            while len(tbd_envs) >0:
                r_o = torch.empty(len(tbd_envs), device=self.device)
                in_obst = torch.zeros_like(self.pos_command_w[:,0],dtype=torch.bool)
                self.pos_command_w[tbd_envs, 0] = self._env.scene.env_origins[tbd_envs,0] + r_o.uniform_(*self.cfg.ranges.pos_x)
                self.pos_command_w[tbd_envs, 1] = self._env.scene.env_origins[tbd_envs,1] + r_o.uniform_(*self.cfg.ranges.pos_y)
                for _obj in range(self.cfg.object_nums):
                    object_pos_w = self._env.scene["object_collection"].data.object_pos_w[tbd_envs,_obj,0:2]
                    _dist = torch.norm(self.pos_command_w[tbd_envs, 0:2] - object_pos_w, dim=-1)
                    in_obst[tbd_envs] = torch.logical_or(in_obst[tbd_envs], _dist<0.4*1.1+0.31415926)
